...
from cloudwright.exporter.terraform.common import _REQUIRED_PROVIDERS, _hcl_quote
...
def _module_blocks(spec: "ArchSpec") -> tuple[list[str], set[str]]:
    ...
    for instance_id, instance in sorted(instances.items(), key=lambda item: item[0]):
        ...
        lines = [
            f'module "{label}" {{',
            f"  source = {_hcl_quote(terraform['source'])}",
        ]
        if terraform.get("version"):
            lines.append(f"  version = {_hcl_quote(terraform['version'])}")
        lines.append("}")
        rendered.append("\n".join(lines))
...
def render(spec: "ArchSpec") -> str:
    ...
    # Strip newlines/control chars from name in this comment to avoid embedding
    # injection markers in the generated header.
    safe_name = (spec.name or "").replace("\n", " ").replace("\r", " ")
    parts.append(f"# Architecture: {safe_name}")
    parts.append("")
    ...
    parts.append("}")
    parts.append("")
    parts.append('variable "lambda_deployment_package" {')
    parts.append('  description = "Path to the Lambda deployment .zip file"')
    parts.append('  default     = "lambda.zip"')
    parts.append("}")
    parts.append("")
    ...
    parts.append(f"  value = {_hcl_quote(spec.name)}")
    parts.append("}")
