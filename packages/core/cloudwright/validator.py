...
from cloudwright.spec import ArchSpec, ValidationCheck, ValidationResult
...
_ALERTING_SERVICES = {"sns", "eventbridge", "pub_sub", "event_hubs", "event_grid"}
...
class Validator:
    def validate(
        ...
        if "FEDRAMP" in frameworks:
            results.append(_check_fedramp(spec))
        if "GDPR" in frameworks:
            results.append(_check_gdpr(spec))
        if well_architected:
...
def _get_regions(spec: ArchSpec) -> list[str]:
    """Get the regions from constraints or fall back to spec.region."""
    if spec.constraints and spec.constraints.regions:
        return spec.constraints.regions
    return [spec.region]


def _check_fedramp(spec: ArchSpec) -> ValidationResult:
    svcs = _services(spec)
    checks = []
    ...
    checks.append(
        ValidationCheck(
            ...
            severity="critical",
            detail=(
                "All data stores have FIPS-compliant encryption"
                if not unencrypted
                else f"Missing encryption on: {', '.join(unencrypted)}"
            ),
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_auth,
            severity="critical",
            detail="MFA-capable auth service present" if has_auth else "No authentication service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_logging,
            severity="critical",
            detail="Audit logging service present" if has_logging else "No audit logging service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_auth,
            severity="high",
            detail="Access control service present" if has_auth else "No IAM or access control service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_monitoring,
            severity="high",
            detail="Monitoring service present" if has_monitoring else "No continuous monitoring service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_alerting,
            severity="medium",
            detail="Incident alerting service present" if has_alerting else "No alerting or notification service found",
...
def _check_gdpr(spec: ArchSpec) -> ValidationResult:
    svcs = _services(spec)
    checks = []
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=len(non_eu) == 0,
            severity="critical",
            detail=("All regions are within the EU" if not non_eu else f"Non-EU regions detected: {', '.join(non_eu)}"),
    ...
    checks.append(
        ValidationCheck(
            ...
            detail=(
                ...
                if not unencrypted
                else f"Unencrypted data stores: {', '.join(unencrypted)}"
            ),
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=in_transit,
            severity="critical",
            detail=("All connections use encrypted protocols" if in_transit else "Unencrypted connections detected"),
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_auth,
            severity="high",
            detail="Access control service present" if has_auth else "No authentication service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            passed=has_logging,
            severity="high",
            detail="Audit trail service present" if has_logging else "No logging or monitoring service found",
    ...
    checks.append(
        ValidationCheck(
            ...
            severity="medium",
            detail=(
                "Data deletion/retention policy configured"
                if has_deletion
                else "No TTL, lifecycle, or retention policy found on data stores"
            ),
